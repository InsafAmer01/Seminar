            ...
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
              ...
              <?php 
                if($_SESSION['accountType']=='market'){
                  ?>
                  <li class="nav-item">
                <a class="nav-link p-2 p-lg-3 active" aria-current="page" href="trackingMarket.php">تتبع طلبات متجرك</a>
              </li>
              <?php 
                }
              ?>
      ...
      <div class="card ">
        <table class="table">
            ...
            <tr >
              ...
              <th class="p-3"scope="col">اسم المشتري</th>
              <th class="p-3"scope="col">السعر</th>
              <th class="p-3"scope="col">الحالة</th>
              <th class="p-3"scope="col">تغيير الحالة</th>س
            </tr>
          </thead>
          <tbody>
                <?php 
            		$id = $_SESSION['userid'];
              		$pdo = new PDO(DBCONNSTRING,DBUSER,DBPASS);
              		$pdo->setAttribute(PDO:: ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
              		$sql="SELECT * FROM trackings as t inner join products as p  where t.productid = p.id";
                    ...
                    $sql2="SELECT * FROM trackings as t inner join users as u  where t.productid=".$row['id']." AND  t.userid = u.id AND t.order_id=".$row['order_id'];
                    ...
                    while ($row2=$result2->fetch()) {
                      echo "<td>".$row2['name']."</td>";
                  		...
                  		if($row['status']=='Pending'){
                  			...
                  			<input type='submit' name='changeStatus' value='shipping' >
                  			...
                  			echo "<td><form method='post' action='".$PHP_SELF."'> 
                  			<input type='submit' name='changeStatus' value='shipping' disabled='disabled'>
                  			<input type='hidden' name='orderid' value='".$row['order_id']."'>
                  			<input type='hidden' name='pid' value='".$pid."'>
                  		</form></td>";
                  		} 
