...
if(isset($_POST['submit'])){
    ...
    $sql="UPDATE products SET name='$name' , price=$price, description='$description',stock=$stock,productType='$productType',categoryName='$category',brandName='$brand' WHERE id=$prodID ";
    $result=$pdo->exec($sql);
        ...
        <div class="container mt-5 mb-5">
            ...
            <form action="<?php echo $PHP_SELF; ?>" method="POST">
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="productName" name="name" required="required" value="<?php echo $row['name'] ?>">
              </div>
              <div class="form-group">
                <label for="productDescription">وصف المنتج</label>
                <textarea class="form-control" id="productDescription" name="description" placeholder="Enter product description" required="required" ><?php echo $row['description'] ?></textarea>
              </div>
              <div class="form-group">
                <label for="productPrice">السعر</label>
                <input type="number" class="form-control" id="productPrice" name="price"placeholder="Enter product price" required="required" value="<?php echo $row['price'] ?>">
              </div>
              <div class="form-group">
                <label for="productStock">عدد القطع في المخزن</label>
                <input type="number" class="form-control" id="productStock" name="stock" placeholder="Enter number of items in stock" required="required" value="<?php echo $row['stock'] ?>">
              </div>
              <div class="form-outline mb-4 mt-3" style="display: block;">
              <h5>الحالة</h5>
                 <div>
                  <select id="condition" name=productType class="m-2">
                  <option value="new">جديد</option>
                  <option value="used">مستخدم</option>
                  </select>
                    <!--label class="block"><input type="radio" name="productType" value="New" required="required">&nbsp;New Product</label>
                    <label class="block ms-5"><input type="radio" name="productType" value="Used" required="required">&nbsp;Used product</label-->
                 </div>
              </div>  
              <div class="form-group">
                <label for="productBrand">العلامة التجارية</label>
                    <select name="productBrand" class="form-control" id="productBrand" placeholder="Enter the brand"required="required">
                      <option selected><?php echo $row['brandName'] ?></option>
                      <option value="Acura">Acura</option>
                      <option value="Alfa Romeo">Alfa Romeo</option>
                      <option value="Aston Martin">Aston Martin</option>
                      <option value="Audi">Audi</option>
              ...
              <div class="form-group">
                <label for="productCategory">الفئة</label>
                    <select name="productCategory" class="form-control" id="productCategory" placeholder="Enter the category" required="required">
                     <option>زيوت</option>
                     <option>مكابح</option>
            ...
            <?php }?>
              <button type="submit" name="submit"class="btn btn-lg">تحديث</button>
            </form>
        </div>
    </section>
